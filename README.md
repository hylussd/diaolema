...
| ⭐ AI推荐 | 规则引擎推荐，输出推荐理由 |
| 📋 打卡记录 | 鱼种/重量/天气/钓法关联记录 |
| 🌊 水文众包 | 水温/溶氧量/鱼种上报（含防垃圾频率限制） |
| 🧪 配方管理 | 气压+水温+鱼种+钓场类型组合，公开/私有 |
| 🛒 商城 | 商品分类/购物车/订单/模拟支付 |
...
| 水文上报 | `GET/POST /api/crowd-reports` | 水文数据查询/上报 |
| 配方 | `GET/POST/PUT/DELETE /api/recipes` | 配方 CRUD（公开/私有） |
| 评分点赞 | `GET/POST /api/spot-ratings` | 1-5星评分 + 点赞 |
| 商城分类 | `GET /api/shop/categories` | 商品分类列表 |
| 商品 | `GET /api/shop/products` | 商品列表/详情 |
| 购物车 | `GET/POST/PUT/DELETE /api/shop/cart` | 购物车 CRUD |
| 订单 | `GET/POST /api/shop/orders` | 订单列表/创建 |
...
### 2026-04-17 — P3 商城
- 6 个商品分类（饵料窝料/鱼钩鱼线/浮漂钓组/钓竿钓椅/配件工具/服装帽子）
- 商品列表 + 商品详情（含规格选择）
- 购物车（添加/增减数量/删除/清空，含库存校验）
- 订单流程（创建/列表/详情/模拟支付/取消，含原子扣库存）
- 7 个前端页面（商城首页/商品列表/商品详情/购物车/确认订单/订单列表/订单详情）
- 会员+积分接口预留（P4）

### 2026-04-15 — P2 社区铺垫
- 水文众包上报（水温/溶氧量/鱼种，需关联最近24h内打卡，含每日3次频率限制）
- 个人配方管理（气压+水温+鱼种+钓场类型组合，公开/私有切换）
- 钓点评分/点赞（1-5星，支持 upsert，评分汇总展示）
- 打卡扩展字段（钓法选择 + 公开开关）
- 标点详情增强（评分星星UI + 点赞按钮 + 公开打卡记录 + 水文数据展示）
- 新增"我的配方"页和"水文上报"页
